...
class Scrublet():
    def __init__(self, counts_matrix, doublets_matrix=None, total_counts=None, sim_doublet_ratio=2.0, n_neighbors=None, expected_doublet_rate=0.1, stdev_doublet_rate=0.02, random_state=0):
        ...
        doublets_matrix : scipy sparse matrix or ndarray, shape (n_cells, n_genes)
            Matrix containing raw (unnormalized) UMI-based transcript counts 
            from cell hashing-derived doublets.
            Converted into a scipy.sparse.csc_matrix.
        ...
        if doublets_matrix is not None:
            if not scipy.sparse.issparse(doublets_matrix):
                doublets_matrix = scipy.sparse.csc_matrix(doublets_matrix)
            elif not scipy.sparse.isspmatrix_csc(doublets_matrix):
                doublets_matrix = doublets_matrix.tocsc()                
            self._E_sim = doublets_matrix
        else:
            self._E_sim = None
        ...
        if doublets_matrix is not None:
            self._total_counts_sim = self._E_sim.sum(1).A.squeeze()
